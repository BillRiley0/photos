...
class Configuration {
  ...
  static const localEntityEncryptionKeyKey = "local_entity_encryption_key";
  static const localUserIDKey = "local_user_id";
  ...
  Future<void> logout({bool autoLogout = false}) async {
    ...
    if (!autoLogout) {
      ...
      if (!isLocalGalleryMode && SyncService.instance.isSyncInProgress()) {
    ...
    if (!autoLogout) {
      ...
      FileUploader.instance.clearCachedUploadURLs();
        ...
        _logger.info("FileUploader not initialized in offline mode", e);
      }
  ...
  String? getToken() {
    // OFFLINE MODE: Always return a token
    _cachedToken ??= _preferences.getString(tokenKey) ?? "offline-token";
    return _cachedToken;
  }

  bool isLoggedIn() {
    // OFFLINE MODE: Always logged in
    return true;
  }
  ...
  String? getEmail() {
    return _preferences.getString(emailKey) ?? "local@offline.gallery";
  }
  ...
  int? getUserID() {
    // OFFLINE MODE: Return local user ID
    return _localUserID ?? _preferences.getInt(userIDKey);
  }
  ...
  Uint8List? getLocalEntityKey() {
    if (_localEntityKey == null) {
      final b64 = _preferences.getString(localEntityEncryptionKeyKey);
      if (b64 != null) {
        _localEntityKey = CryptoUtil.base642bin(b64);
      } else {
        _localEntityKey = CryptoUtil.generateKey();
        _preferences.setString(
          localEntityEncryptionKeyKey,
          CryptoUtil.bin2base64(_localEntityKey!),
        );
      }
    }
    return _localEntityKey;
  }

  Uint8List? getKey() {
    // OFFLINE MODE: Always return a valid key
    if (_key == null) {
      _key = _preferences.getString(localEntityEncryptionKeyKey);
      if (_key == null) {
        _key = CryptoUtil.bin2base64(CryptoUtil.generateKey());
        _preferences.setString(localEntityEncryptionKeyKey, _key!);
      }
    }
  ...
  bool hasConfiguredAccount() {
    // OFFLINE MODE: Always configured
    return true;
  }

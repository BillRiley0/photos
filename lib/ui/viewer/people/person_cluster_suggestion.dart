...
import "package:photos/db/files_db.dart" as offline_db;
...
class _PersonClustersState extends State<PersonReviewClusterSuggestion> {
              ...
              if (event.source == clusterID.toString()) {
                ...
                if (event.type == PeopleEventType.removedFaceFromCluster) {
                  for (final String removedFaceID in event.relevantFaceIDs!) {
                    ...
                    files.removeWhere((file) => file.uploadedFileID == fileID || _resolveThumbnailKey(file) == fileID);
                  }
                  setState(() {});
                }
              }
            ...
            children: [
              FaceThumbnailSquircleClip(
                child: FileFaceWidget(
                  ...
                  clusterID: cluserId,
                  faceCrop:
                      faceThumbnails[_resolveThumbnailKey(files[start + index])],
                ),
              ),
  ...
  /// Resolves a file to its integer key for thumbnail map lookup.
  /// Works with both uploadedFileID (server mode) and localID (offline mode).
  int _resolveThumbnailKey(EnteFile file) {
    return file.uploadedFileID ?? file.localID.hashCode;
  }
  ...
  Future<Map<int, Uint8List?>> _generateFaceThumbnails(
    ...
    for (var i = 0; i < faceCropsList.length; i++) {
      faceCrops[_resolveThumbnailKey(files[i])] = faceCropsList[i];
    }
    return faceCrops;
  }

...
import 'package:photos/core/configuration.dart' show Configuration;
...
class EntityService {
  ...
  Future<LocalEntityData> addOrUpdate(
    ...
    // OFFLINE MODE: Store locally without server
    if (isLocalGalleryMode) {
      return _addOrUpdateLocal(type, jsonMap, id: id);
    }
  ...
  /// Local-only entity storage. Generates UUIDs, encrypts with a locally-stored
  /// key, and writes directly to the SQLite DB without touching the server.
  Future<LocalEntityData> _addOrUpdateLocal(
    EntityType type,
    Map<String, dynamic> jsonMap, {
    String? id,
  }) async {
    final String plainText = jsonEncode(jsonMap);
    final key = await _getLocalEntityKey(type);
    ...
    final localData = LocalEntityData(
      ...
      type: type,
      data: plainText,
      ownerID: Configuration.instance.getUserID() ?? 0,
      updatedAt: now,
    );

    await _db.upsertEntities([localData]);
    _logger.info(
      "[LOCAL] ${id == null ? 'Created' : 'Updated'} entity $entityId of type ${type.name}",
    );
    return localData;
  }

  /// Returns a locally-generated encryption key for entities, stored in
  /// SharedPreferences keyed by entity type. No server round-trip.
  Future<Uint8List> _getLocalEntityKey(EntityType type) async {
    final localKey = Configuration.instance.getLocalEntityKey();
    if (localKey == null) {
      throw Exception("Local entity encryption key not initialized");
    }
    return localKey;
  }
  ...
  Future<void> deleteEntry(String id) async {
    if (isLocalGalleryMode) {
      _logger.info("[LOCAL] Deleting entity $id");
  ...
  Future<void> syncEntities() async {
    if (isLocalGalleryMode) {
      _logger.finest("[LOCAL] Skipping server sync for entities");
      return;
    }
    try {
  ...
  Future<int> syncEntity(EntityType type) async {
    if (isLocalGalleryMode) {
      _logger.finest("[LOCAL] Skipping server sync for entity type ${type.name}");
      return 0;
    }
    try {
  ...
  Future<Uint8List> getOrCreateEntityKey(EntityType type) async {
    // OFFLINE MODE: Use local key
    if (isLocalGalleryMode) {
      return _getLocalEntityKey(type);
    }

...
class PersonService {
  ...
  /// Resolves a local file ID string to the integer ID used in the ML database.
  Future<int> _getLocalIntFileId(String localID) async {
    try {
      return await OfflineFilesDB.instance.getOrCreateLocalIntId(localID);
    } catch (e) {
      logger.warning("Failed to get local int file ID, using hashCode fallback", e);
  ...
  Future<bool> fetchRemoteClusterFeedback({
    ...
    // OFFLINE MODE: Use local DB for cluster feedback
    if (isLocalGalleryMode) {
      logger.finest("Using local cluster feedback in offline mode");
      return true;
    }
  ...
  Future<PersonEntity> updateAvatar(PersonEntity p, EnteFile file) async {
    // OFFLINE MODE: Use local file ID when uploadedFileID is null
    final int fileID;
    if (file.uploadedFileID != null) {
      fileID = file.uploadedFileID!;
    } else if (file.localID != null && file.localID!.isNotEmpty) {
      fileID = await _getLocalIntFileId(file.localID!);
    } else {
      throw Exception(
        "No valid file ID for person ${p.remoteID}",
      );
    }
    ...
    if (face == null) {
      throw Exception(
        "No face found for person ${p.remoteID} in file $fileID",
      );
    }

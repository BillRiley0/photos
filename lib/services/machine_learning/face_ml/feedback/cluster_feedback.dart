...
class ClusterFeedbackService<T> {
  ...
  Future<List<ClusterSuggestion>> getSuggestionForPerson(
    ...
    // OFFLINE MODE: Allow person suggestions in local gallery mode
    if (isLocalGalleryMode) {
      _logger.info(
        "Running getSuggestionForPerson in local gallery mode",
      );
    }
    _logger.info(
  ...
  Future<List<(String, double, String)>> _getFastSuggestions(
    ...
    // OFFLINE MODE: Allow fast suggestions in local gallery mode
    if (isLocalGalleryMode) {
      _logger.info(
        "Running _getFastSuggestions in local gallery mode",
      );
    }
    final allClusterIdsToCountMap = (await mlDataDB.clusterIdToFaceCount());
  ...
  /// Resolves a file to its integer ID for map lookups.
  /// Falls back to localID.hashCode in offline mode.
  int _resolveFileID(dynamic file) {
    return (file.uploadedFileID as int?) ?? file.localID.hashCode;
  }
  Future<void> _sortSuggestionsOnDistanceToPerson(
    ...
    for (final suggestion in suggestions) {
      ...
      suggestion.filesInCluster.sort((b, a) {
        ...
        final double distanceA = fileIdToDistanceMap[_resolveFileID(a)] ?? -1;
        final double distanceB = fileIdToDistanceMap[_resolveFileID(b)] ?? -1;
        return distanceA.compareTo(distanceB);
      });
      w?.log('sorted files for cluster $clusterID');

      debugPrint(
        "[${_logger.name}] Sorted suggestions for cluster $clusterID based on distance to person: ${suggestion.filesInCluster.map((e) => fileIdToDistanceMap[_resolveFileID(e)]).toList()}",
      );
    }

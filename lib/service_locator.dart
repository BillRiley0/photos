...
class ServiceLocator {
  ...
  /// OFFLINE MODE: Initialize with stub network clients that never connect
  /// to any server. All gateways will fail gracefully if called.
  void initOffline(
    SharedPreferences prefs,
    PackageInfo packageInfo,
  ) {
    this.prefs = prefs;
    // Stub Dios that point nowhere - gateways will fail if called,
    // but we've removed all server callers in offline mode
    this.enteDio = Dio();
    this.nonEnteDio = Dio();
    this.downloadDio = Dio();
    this.packageInfo = packageInfo;
    endpointConfig = EndpointConfig(prefs);
  }
}
...
// OFFLINE MODE: Always in local gallery mode
bool get isLocalGalleryMode => true;
